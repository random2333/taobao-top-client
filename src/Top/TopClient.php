...
class TopClient
{
	...
	protected function logCommunicationError($apiName, $requestUrl, $errorCode, $responseTxt)
	{
		...
		$logger->conf["log_file"] = rtrim(env('TOP_SDK_WORK_DIR','/tmp/'), '\\/') . '/' . "logs/top_comm_err_" . $this->appkey . "_" . date("Y-m-d") . ".log";
	...
	public function execute($request, $session = null,$bestUrl = null)
	{
		...
		{
			$logger = new TopLogger;
			$logger->conf["log_file"] = rtrim(env('TOP_SDK_WORK_DIR','/tmp/'), '\\/') . '/' . "logs/top_biz_err_" . $this->appkey . "_" . date("Y-m-d") . ".log";
			$logger->log(array(
				date("Y-m-d H:i:s"),
				$resp
			));
